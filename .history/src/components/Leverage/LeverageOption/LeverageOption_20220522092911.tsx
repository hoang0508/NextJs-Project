...
import {
  Progress,
  ProgressState,
} from "@components/WrappedActionMenu/Progress/Progress";
...
const LeverageOption: React.FC<IProp> = ({
  onClickSubmitButton,
  onClickResetButton,
  minCRatio,
  liquidationPriceTo,
  ...
  return (
    <Grid
      className={classNames(
        classes.wrapperOption,
        action === "close" ? classes.blur : null
      )}
    >
      <Grid className={classes.root}>
            ...
            <Grid>
              ...
              <Typography className={classes.headerTitle}>
                ...
                {Number(liquidationPriceTo) > 99999
                  ? Number(
                      Number(liquidationPriceTo).toFixed(0)
                    ).toLocaleString("pl-PL")
                  : Number(liquidationPriceTo)
                      .toLocaleString("pl-PL")
                      .replace(",", ".")}
              </Typography>
            </Grid>
        ...
        >
          <Progress
            ...
            state={showOperationProgressFinale}
          />
          <OutlinedButton
            name={"Reset all"}
              ...
              <MobileTooltip
                hint={
                  <>
                    ...
                    There is a risk of loss and a possibility of higher profits.
                    Use it responsibly.{" "}
                    {showWarning.status ? (
                      <strong style={{ fontWeight: 900 }}>
                        ...
                        {showWarning.tokenFrom} for {showWarning.tokenTo} from
                        the existing pair.
                      </strong>
                    ) : null}
                  </>

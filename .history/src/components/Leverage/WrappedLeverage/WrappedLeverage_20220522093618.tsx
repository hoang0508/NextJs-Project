...
import { IActionContents } from "../LeverageAction/LeverageAction";
import LeverageAction from "../LeverageAction/LeverageAction";
...
const WrappedLeverage: React.FC<IProp> = ({
  allSynthetic,
  pairs,
  sending,
  hasError,
  ...
  const [blockSubmitButton, setBlockSubmitButton] =
    React.useState<boolean>(true);
  const [blockCloseButton, setBlockCloseButton] = React.useState<boolean>(true);

  const [availableToClosePosition, setAvailableToClosePosition] =
    React.useState<boolean>(false);
  const [userVaultIndex, setUserVaultIndex] =
  ...
  const [currentLeverageTable, setCurrentLeverageTable] = React.useState<
    ILeveragePair[]
  ...
  React.useEffect(() => {
    if (
      ...
      const collateral = {
        price:
          assetPrices[
            allSynthetic[pairIndex].syntheticData.assetAddress.toString()
          ].val,
        ...
        setBlockCloseButton(
          !synthetic.syntheticData.balance
            .add(
              calculateAmountAfterSwap(
                assetPrices[
                  currentLeverageTable[leverageIndex].collateral.toString()
                ].val,
                assetPrices[
                  currentLeverageTable[leverageIndex].synthetic.toString()
                ].val,
                ...
                calculateAvailableWithdraw(
                  ...
                  .mul(
                    new BN(
                      Number(0.9) *
                        10 **
                          currentLeverageTable[leverageIndex].collateralAmount
                            .scale
                    )
                  )
                  .div(
            ...
            .gt(
              closeAmount
                .mul(
                  new BN(
                    Number(0.3) *
                      10 ** currentLeverageTable[leverageIndex].maxBorrow.scale
                  )
                )
                .div(
                  new BN(
      ...
      <CloseLeverage
        ...
        amount={
          currentLeverageTable.length > 0 && leverageIndex !== null
            ? {
                val: closeAmount,
                scale: currentLeverageTable[leverageIndex].maxBorrow.scale,
              }
  ...
  return (
    <Grid className={classes.root}>
      <Grid className={classes.actionGrid}>
        ...
        {userVaults.filter(
          (element) =>
            element.collateral[0] === "x" && element.borrowed[0] === "x"
        ).length !== 0 ? (
          <BorrowTable
...
export default WrappedLeverage;

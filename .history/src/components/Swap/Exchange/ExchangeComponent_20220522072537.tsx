...
export const calculateSwapOutAmount = (
  ...
  if (decimalChange < 1) {
    return printBN(
      amountAfterFee.div(new BN(1 / decimalChange)),
      assetFor.supply.scale
    );
  } else {
    return printBN(
      amountAfterFee.mul(new BN(decimalChange)),
      assetFor.supply.scale
    );
  }
};
export const calculateEstimateAmount = (
  ...
  if (decimalChange < 1) {
    return printBN(
      actualAmountOut.div(new BN(1 / decimalChange)),
      assetFor.supply.scale
    );
  } else {
    return printBN(
      actualAmountOut.mul(new BN(decimalChange)),
      assetFor.supply.scale
    );
  }
};
...
export const calculateSwapOutAmountReversed = (
  ...
  const amountOutBeforeFee = assetFor.price.val
    .mul(amountAfterFee)
    .div(assetIn.price.val);

  const decimalChange = 10 ** (assetFor.supply.scale - assetIn.supply.scale);

  if (decimalChange < 1) {
    return printBN(
      amountOutBeforeFee.mul(new BN(1 / decimalChange)),
      assetIn.supply.scale
    );
  } else {
    return printBN(
      amountOutBeforeFee.div(new BN(decimalChange)),
      assetIn.supply.scale
    );
  }
};
...
export const swapOutAmountCurrencyName = (
  ...
  const firstSymbol =
    tokenToIndex === null ? "" : `${tokens[tokenToIndex].symbol} `;
  const secondSymbol =
    tokenFromIndex === null ? "" : `${tokens[tokenFromIndex].symbol} `;
  return reserved
...
const ExchangeComponent: React.FC<IExchangeComponent> = ({
  tokens,
  onSwap,
  fee,
  discountPercent,
  ...
  const getButtonMessage = (
    ...
    if (
      tokenToIndex !== null &&
      printBNtoBN(amountTo, tokenTo.supply.scale).gt(
        tokenTo.maxSupply.val.sub(tokenTo.supply.val)
      )
    ) {
      return (
        <>
          ...
          <MobileTooltip
            hint={
              <>
                ...
                Your amount exceeded current supply of token. Available to
                trade:
                <b style={{ wordWrap: "break-word" }}>{` ${printBN(
                  tokens[tokenToIndex].maxSupply.val.sub(
                    tokens[tokenToIndex].supply.val
                  ),
  ...
  return (
    <Grid container className={classes.root} direction="column">
      <Grid
        ...
        container
        direction="column"
        className={classes.tokenComponent}
      >
        <Grid
          item
          container
        ...
        <ExchangeAmountInput
          ...
          current={
            tokenFromIndex !== null ? tokens[tokenFromIndex].symbol : null
          }
        ...
        >
          ...
          >
            ...
            <MobileTooltip
              hint={
                <>
                  <Image src={Output} alt="" className={classes.outputIcon} />
                  <Typography className={classes.tooltipTitle}>
                    Estimated output amount
                  </Typography>
                  <p style={{ marginBlock: 10, color: colors.navy.lightGrey }}>
                    Output amount is calculated based on the most up-to-date
                    data from price oracles, so it can change due to the
                    sub-second update intervals of the oracles.
                  </p>
      ...
      <Grid container item className={classes.numbersField}>
        <Grid className={classes.numbersFieldGrid} item>
          ...
          >
            ...
            <MobileTooltip
              hint={
                <>
                  <Image src={Fee} alt="" className={classes.circleIcon} />
                  <Typography className={classes.tooltipTitle}>
                    Fee tiers
                  </Typography>
                  <p style={{ marginBlock: 10, color: colors.navy.lightGrey }}>
                    You can gain discounts on the swap fee by depositing SNY to
                    Synthetify Exchange. Your current discount on the fee is{" "}
          ...
          >
            <Typography
              className={classes.numbersFieldAmount}
        ...
        <Grid className={classes.numbersFieldGrid} item>
          <Grid container item alignItems="center">
            <Typography className={classes.numbersFieldTitle}>
              Exchange rate
            </Typography>

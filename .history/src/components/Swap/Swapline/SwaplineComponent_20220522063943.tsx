...
export const getAssetInAndFor = (
  ...
  const synthetic: AssetPriceData = {
    ...
    maxAvailable: pair.syntheticData.maxSupply.val.sub(
      pair.syntheticData.supply.val
    ),
...
export const calculateSwapOutAmountReversed = (
  ...
  const amountOutBeforeFee = assetFor.priceVal
    .mul(amountAfterFee)
    .div(assetIn.priceVal);
...
export const SwaplineComponent: React.FC<ISwaplineComponent> = ({
  ...
  const [pairIndex, setPairIndex] = React.useState<number | null>(
    pairs.length ? 0 : null
  );
  const [swapType, setSwapType] =
    React.useState<SwaplineSwapType>("nativeToSynthetic");
      ...
      <Grid container item className={classes.numbersField}>
        <Grid item className={classes.numbersFieldGrid}>
          ...
          <Typography className={classes.numbersFieldAmount}>
            {pairIndex !== null
              ? +printBN(
                  pairs[pairIndex].fee.val.mul(new BN(100)),
                  pairs[pairIndex].fee.scale
                )
              : 0}
            %
          </Typography>
